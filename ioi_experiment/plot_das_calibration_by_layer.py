"""Render a DAS calibration heatmap (layer x subspace_dim) from a saved IOI DAS JSON payload."""
...
INPUT_PATH = Path("results/ioi_run_results.json")
OUTPUT_PATH = INPUT_PATH.with_name("das_answer_index_calibration_heatmap.svg")
TARGET_VAR = "answer_index"
...
def render_svg(layers: list[int], subspace_dims: list[int], values: dict[tuple[int, int], float], output_path: Path) -> None:
    ...
    lines = [
        ...
        '<text x="100" y="32" font-family="Helvetica, Arial, sans-serif" font-size="22" font-weight="700">DAS Max Calibration Accuracy Heatmap (answer_index)</text>',
        '<text x="100" y="54" font-family="Helvetica, Arial, sans-serif" font-size="13" fill="#4b5563">Color shows max calibration choice accuracy for each (layer, subspace_dim) candidate.</text>',
    ]
