"""DAS search over IOI residual-stream sites."""
...
from .data import IOIPairBank, IOIPairDataset
...
class DASConfig:
    """Hyperparameters controlling IOI DAS search."""
...
def train_das_candidate(
    *,
    ...
    intervention = DASSubspaceIntervention(
        hidden_size=int(model.config.hidden_size), subspace_dim=int(subspace_dim)
    ).to(device)
    optimizer = torch.optim.Adam(intervention.parameters(), lr=float(learning_rate))
    loader = DataLoader(IOIPairDataset(bank), batch_size=batch_size, shuffle=True)
    losses: list[float] = []
    previous_loss = None
    plateau_steps = 0
    for epoch_index in range(int(max_epochs)):
        ...
        for batch in loader:
            ...
            answer_token_ids = batch["answer_token_id"].view(-1).to(logits.device)
            loss = F.cross_entropy(logits, answer_token_ids)
...
def evaluate_das_candidate(
    *,
    ...
    loader = DataLoader(IOIPairDataset(bank), batch_size=batch_size, shuffle=False)
    logits_all = []
    for batch in loader:
        with torch.no_grad():
            logits = run_das_residual_intervention(
    ...
    if return_details:
        metrics["prediction_details"] = das_prediction_details_from_logits(
            full_logits, bank, tokenizer=tokenizer
        )
    return metrics
...
def run_das_pipeline(
    *,
    ...
    subspace_dims = (
        tuple(range(1, hidden_size + 1)) if config.subspace_dims is None else tuple(config.subspace_dims)
    )
    search_records = []
    best = None
    ...
    for site in sites:
        for subspace_dim in subspace_dims:
            ...
            record: dict[str, object] = {
                ...
                "calibration_choice_acc": float(calibration_metrics["choice_acc"]),
            ...
            if holdout_metrics_for_candidate is not None:
                ...
                record["holdout_choice_acc"] = float(holdout_metrics_for_candidate["choice_acc"])
                if "decoded_answer_acc" in holdout_metrics_for_candidate:
                    record["holdout_decoded_answer_acc"] = float(
                        holdout_metrics_for_candidate["decoded_answer_acc"]
                    )
            ...
            if config.verbose:
                message = (
                    ...
                    f"choice_acc={float(calibration_metrics['choice_acc']):.4f}"
                )
                if holdout_metrics_for_candidate is not None:
                    message += (
                        f" holdout_exact_acc={float(holdout_metrics_for_candidate['exact_acc']):.4f}"
                        f" holdout_choice_acc={float(holdout_metrics_for_candidate['choice_acc']):.4f}"
                    )
                print(message)
    ...
    if config.verbose:
        print(
            ...
            f"choice_acc={float(holdout_metrics['choice_acc']):.4f}"
        )
    result = {
        **best,
