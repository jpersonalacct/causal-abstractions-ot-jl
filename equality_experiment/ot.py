...
def sinkhorn_unbalanced_ot(
    ...
    """Repo-style entropic unbalanced OT with KL penalties on marginals."""
    if epsilon <= 0:
        raise ValueError("epsilon must be > 0")
    if temperature <= 0:
        raise ValueError("temperature must be > 0")
...
def _sinkhorn_from_cost(
    ...
    b = b.to(dtype=torch.float32, device=cost.device)
    kernel = torch.exp(-cost / (epsilon * temperature)).clamp_min(1e-30)
...
def _gw_linearized_cost_square(
    cost_var: torch.Tensor,
    cost_site: torch.Tensor,
    p: torch.Tensor,
    q: torch.Tensor,
    transport: torch.Tensor,
) -> torch.Tensor:
    """Compute the square-loss GW linearized cost matrix."""
    const_var = (cost_var.pow(2) @ p).unsqueeze(1)
    const_site = (cost_site.pow(2) @ q).unsqueeze(0)
    coupling_term = cost_var @ transport @ cost_site.transpose(0, 1)
    return const_var + const_site - 2.0 * coupling_term


def _gw_objective_square(
    cost_var: torch.Tensor,
    cost_site: torch.Tensor,
    p: torch.Tensor,
    q: torch.Tensor,
    ...
    """Evaluate the square-loss GW objective for a transport plan."""
    linearized_cost = _gw_linearized_cost_square(cost_var, cost_site, p, q, transport)
    return float((linearized_cost * transport).sum().item())
...
def choose_better_variable_candidate(
    ...
    if incumbent is None:
        return True
    return float(candidate["exact_acc"]) > float(incumbent["exact_acc"])
...
def run_alignment_pipeline(
    ...
    calibration_selection = select_transport_hyperparameters(
    ...
    selected_top_k_by_variable = calibration_selection["selected_top_k_by_variable"]
    selected_lambda_by_variable = calibration_selection["selected_lambda_by_variable"]
    per_variable_top_k = [selected_top_k_by_variable[variable] for variable in config.target_vars]
    truncated_transport = truncate_transport_rows(
    ...
    for record in final_results:
        ...
        selected_calibration_records = {
            str(item["variable"]): item for item in calibration_selection["results"]
        }
        calibration_record = selected_calibration_records[variable]
        record["selection_exact_acc"] = float(calibration_record["exact_acc"])
        record["selection_cross_entropy"] = None
        record["selected_layer"] = rankings[variable][0]["layer"] if rankings[variable] else None
    ...
    for variable, candidates in calibration_selection["search_records"].items():
        ...
        for candidate in candidates:
            ...
            variable_summaries.append(
                {
                    "layer": rankings[variable][0]["layer"] if rankings[variable] else None,
                    "continuous_cutoff": None,
                    "selection_cross_entropy": None,
                    "selection_exact_acc": float(candidate["exact_acc"]),
                    "top_site_label": top_site_label,
                    "top_k": int(candidate["top_k"]),
                    "lambda": float(candidate["lambda"]),
                    "result": result,
                }
            )
        layer_candidate_summaries[variable] = variable_summaries
    ...
    return {
        ...
        "selected_hyperparameters": {
            ...
            "selected_layer_by_variable": {
                variable: (rankings[variable][0]["layer"] if rankings[variable] else None)
                for variable in config.target_vars
            },
            "continuous_cutoff_by_variable": {variable: None for variable in config.target_vars},
        },
        "selection_objective": "calibration_exact",
        "final_evaluation_policy": "soft_transport_top_k",
        "calibration_sweep": calibration_selection["search_records"],
        "layer_candidate_summaries": layer_candidate_summaries,
        "results": final_results,
        "layer_masks_by_variable": {
            variable: {f"L{layer}": mask.tolist() for layer, mask in masks.items()}
            for variable, masks in layer_masks.items()
        },
    }
